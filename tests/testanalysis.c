...
int main(void) {
  ...
  equal_bool(false, test_battery->data->pre_sleep, "Current time is not pre sleep", state);
  ...
  equal_int(1056, test_battery->data->average_sleep_time, "7 standard sleep times + normal non-sleep usage + anomaly",
            state);
  ...
  // 16 June 18.15
  current_time = 1592331300;

  FILE *test_file_4 = fopen("./analysis_tests/analysistest4.csv", "r");

  monitor_sleep_time(current_time, test_battery, test_file_4);

  equal_int(1065, test_battery->data->average_sleep_time,
            "7 standard sleep times + normal non-sleep usage + anomaly + usage before 7 day window", state);

  equal_bool(true, test_battery->data->pre_sleep, "Current time is pre sleep", state);
